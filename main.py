...
async def InsertRetroactiveLeave(ctx, discorduser, leavetype, startdate, enddate, requestedlate, reason = ""):
    message_content = ""
    if Utilities.IsAdmin(ctx.author):
        await ctx.send(content = "Processing")
        message_content = await leave_interface.InsertRetroactiveLeave(discorduser, ctx.message.id, datetime.strptime(startdate, '%d/%m/%Y'), datetime.strptime(enddate, '%d/%m/%Y'), leavetype, requestedlate, reason)
    else:
        message_content = "This command is for Admins only"
