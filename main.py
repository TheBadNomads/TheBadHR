...
client = commands.Bot(command_prefix = "!", intents = discord.Intents(members = True, guilds = True))
...
async def InsertMember(ctx, discorduser, name, email, startdate):
    if Utilities.IsAdmin(ctx.author) or Utilities.IsBotManager(ctx.author):
